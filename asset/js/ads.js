class AdService {
    ...
    async createAd(adData) {
        try {
            ...
            const result = await this.database.createDocument('ads', adDoc);
    ...
    async getAds() {
        try {
            const ads = await this.database.listDocuments('ads');
    ...
    async participateInAd(adId) {
        try {
            ...
            const tx = await this.database.createDocument('transactions', {
                userId: walletService.userAddress,
            ...
            // Simulate verification (in real app, this would be admin action)
            setTimeout(async () => {
                await this.database.updateDocument('transactions', tx.$id, {
                    status: 'completed'
                });
                ...
                await this.database.updateDocument('users', user.$id, {
                    tokenBalance: (userDoc.tokenBalance || 0) + 350
                });
